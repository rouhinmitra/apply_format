...
sys.path.append(
    "D:\\Backup\\Rouhin_Lenovo\\US_project\\GEE_SEBAL_Project\\geeSEBAL_copy_edits\\etbrasil\\"
)
import geesebal
from geesebal import (
    tools,
    landsatcollection,
    masks,
    meteorology,
...
)  # Neutral to accomodate Bi1 and Vaira
...
def runsebal(image):
    ...
    ls_trial = tools.LST_DEM_correction(
        ls_trial, z_alt, T_air, UR, sun_elevation, hour, minuts
    )
    print("It's a miracle")
    ## GET IMAGE
    ## COLD PIXEL
    d_cold_pixel = endmembers.fexp_cold_pixel(
        ls_trial, geometryReducer, p_top_NDVI, p_coldest_Ts
    )
    ...
    ##HOT PIXEL
    d_hot_pixel = endmembers.fexp_hot_pixel(
        ls_trial, geometryReducer, p_lowest_NDVI, p_hottest_Ts
    )
    ##SENSIBLE HEAT FLUX (H) [W M-2]
    ls_trial = tools.fexp_sensible_heat_flux(
        ls_trial, ux, UR, Rn24hobs, n_Ts_cold, d_hot_pixel, date_string, geometryReducer
    )
    ...
    return (
        ls_trial,
        col_meteorology,
        d_cold_pixel.get("temp").getInfo(),
        d_hot_pixel.get("temp").getInfo(),
        d_hot_pixel.get("Rn").getInfo(),
        d_hot_pixel.get("G").getInfo(),
    )
